...
namespace Middleware.Reversal.Services
{
    public class CaseTransmitter : ICaseTransmitter
    {
        ...
        private readonly ILogger<CaseTransmitter> _logger;
        ...
        public CaseTransmitter(IOptions<ProcessMakerSettings> options, ILogger<CaseTransmitter> logger, ICaseDAO caseDao, IDocumentDAO documentDAO)
        {
            _settings = options.Value;
            _logger = logger;
            _caseDao = caseDao;
            _documentDAO = documentDAO;
            bpmCase = new DDL.ProcessMaker.Services.Case(_settings.ServerAddress, _settings.ClientId, _settings.ClientSecret, _settings.UserName, _settings.Password)
            {
        ...
        public async Task TransmitCases()
        {
            ...
            foreach (var c in cases)
            {
                ...
                {
                    _logger.LogInformation($"Error while transmitting {c.AccountId} =>{ex.Message}=>{ex.StackTrace}");
                    continue;
                }
            }
        ...
        public async Task TransmitUpdatedDocuments()
        {
            ...
            try
            {
                ...
                foreach (var doc in documents)
                {
                    ...
                    {
                        _logger.LogInformation($"Error while transmitting {c.AccountId} =>{e.Message}=>{e.StackTrace}");
            ...
            {
                _logger.LogInformation($"Error while get documents for update {ex.Message}=>{ex.StackTrace}");
            }
        }
